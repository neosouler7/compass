...
func subscribeWs(wg *sync.WaitGroup) {
	...
	msg := fmt.Sprintf("[{\"method\": \"subscribe\", \"type\": \"orderbook\", \"symbols\": [%s]}, {\"method\": \"subscribe\", \"type\": \"trade\", \"symbols\": [%s]}]", streams, streams)
...
func processWsMessages(done <-chan struct{}, msgQueue <-chan []byte) {
	for {
		...
		case msgBytes := <-msgQueue:
			if strings.Contains(string(msgBytes), "pong") {
				fmt.Printf("%s PONG\n", exchange)
			} else {
				var rJson interface{}
